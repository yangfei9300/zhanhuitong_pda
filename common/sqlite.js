module.exports = {
  dbName: 'zht', // 数据库名称
  dbPath: '_downloads/zht.db', // 数据库地址,推荐以下划线为开头   _doc/xxx.db
  ...
  selectTableData(dbTable, uname, namevalue, upass, passvalue,urrn,rrnvalue,urrn1,rrnvalue1) {
    if (dbTable !== undefined) {
      // 第一个是表单名称，后两个参数是列表名，用来检索
	  
	  if (uname !== undefined && upass !== undefined && urrn!==undefined && urrn1!==undefined) {
	    // 四个检索条件
	    var sql = `SELECT * FROM ${dbTable} WHERE ${uname} = '${namevalue}' AND ${upass} = '${passvalue}' AND ${urrn}='${rrnvalue}' AND ${urrn1}='${rrnvalue1}' `;
	  }else
