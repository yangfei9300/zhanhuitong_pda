<template>
	<view class="colonn center_center">
		...
		<view class="roww w-710 border_bottom" style="padding: 20rpx 20rpx; font-size: 30rpx">
			...
			<view class="allline"></view>
			<view style="color: #000000; text-align: right" class="w-500 txtShowLength">
				<block v-if="isLixian == 2">离线模式</block>
				<block v-else>在线模式</block>
			</view>
		</view>
		<view class="roww w-710 border_bottom" style="padding: 20rpx 20rpx; font-size: 30rpx">
			<view>懒人模式（连续扫描）</view>
			<view class="allline"></view>
			<view style="color: #000000; text-align: right; font-weight: bold" class="w-300" @click.stop="lianxuIsClick">
				{{ lianxuIs ? '关闭' : '开启' }}
			</view>
		</view>
		...
		<view
			class="btncen center_center colonn"
			style="margin-top: 40%"
			:style="{
				'background-color': bg
			}"
		>
			...
			<view style="color: #ffffff; font-size: 40rpx; line-height: 30rpx; font-weight: bold" class="colonn center_center">
				<view>核销手机号</view>
...
export default {
	...
	onShow() {
		...
		uni.$on('scan', (res) => {
			...
			this.jiemiResult=jiemiResult;
			
			if(jiemiResult.indexOf("https")==-1){
				jiemiResult = this.Decrypt(jiemiResult);
	...
	methods: {
		...
		adderror(txtxtx) {
			...
			DB.insertTableData('scan_error_info', sql, condition)
				.then((res) => {  
					this.$tools.showToast(JSON.stringify(txtxtx)+"新增错误日志成功");
				})
				.catch((error) => {
		...
		continueScanFunc(enable) {
			...
			if (enable) {
				//设置连续扫描间隔时间
				barcodeModel.intervalSet(this.invTime, (ret) => {
		...
		// 核销展会入场
		hexiao(unionid, activityId, type, isLine) {
			// 扫码结果
			var data = {
				exhId: this.exhInfo[1],
				params: {
		...
		selectHexiaoNum() {
			...
			var userInfo = uni.getStorageSync('userInfo');
			DB.selectTableData('statistics_num', 'exhId', exhInfo[1], 'doorId', exhInfo[2], 'loginPhone', userInfo.phoneNum, 'dayTime', time)
				.then((res) => {
