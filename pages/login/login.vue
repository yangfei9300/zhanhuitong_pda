<template>
	<view>
		...
		<view class="colonn center_center pore">
			<view class="roww endend poab" style="margin-top: 30rpx;top: 0rpx;" @click.stop="lixianClick">
			...
			<view class="colonn  p-all-25 ">
				<view class="roww  center_center fs-35 border_bottom m-top-25 p-bottom-20" @longtap="isShowInfoClick">
					<view class="roww center_center">
					...
					<view class="roww center_center">
						...
						<view v-else style="color: red;">请扫码获取展会信息</view>
					</view>
					<view class="allline"></view>
				</view>
				...
				<view class="colonn  fs-35 border_bottom m-top-25 p-bottom-10">
					...
					<input maxlength="11" v-model="form.staffName" class="fs-40 m-left-15" placeholder-class="fs-40"
						placeholder="请输入姓名" />
				</view>
				<view class="colonn  fs-35 border_bottom m-top-25 p-bottom-10">
					<view class="roww  center_center">
					...
					<view class="h-15"></view>
					<input maxlength="11" v-model="form.phoneNum" class="fs-40 m-left-15" placeholder-class="fs-40"
						placeholder="请输入手机号" />
				</view>



				<view class="roww center_center" @click.stop="loginstaffpda">
					<view class="xiayibubtn fs-35">登录</view>
				</view>
				<view class="roww center_center" @click.stop="toYeji(1)">
					<view class="xiayibubtn fs-35">查看业绩</view>
				</view>
				<view class="roww center_center" @longtap="toYeji(2)" v-if="isShowInfo">
					<view class="xiayibubtn fs-35">扫码记录</view>
				</view>
				<view class="roww center_center" @longtap="toUploadPage()" v-if="isShowInfo">
					<view class="xiayibubtn fs-35">离线数据上传</view>
				</view>
				<view class="roww center_center" @longtap="toerrorqr()" v-if="isShowInfo">
					<view class="xiayibubtn fs-35">错误二维码查看</view>  
				</view>
				<view class="h-150"></view>
			</view>
		</view>

		<view class="gengxinbtn" @click.stop="toGemhxom">检查更新</view>

		<view class="gengxinbtn1 yincang1" @longtap="guanliyuanClick">管理员</view>
	...
	export default {
		...
		methods: {
			...
			loginstaffpda() {
				...
				console.log("asdasd", this.isLixian == 2, this.isLixian)
				if (this.isLixian == 2) {
				...
				console.log("请求地址", this.$paths.loginstaffpda)
				this.$axios
			...
			createTable() {
				...
				if (open) {
					...
					DB.createTable("scan_error_info", sql)
						.then(res => {
							console.log("创建scan_error_info表成功", res);
						})
						.catch(error => {
							console.log("创建表scan_error_info失败");
							this.$tools.showToast("创建scan_error_info表失败", error);
						});
			...
			selCountNUm() {
				...
				DB.selectTableData("statistics_num", 'exhId', scanInfo[1], 'doorId', scanInfo[2], 'loginPhone', this.form
						.phoneNum, 'dayTime', time)
					.then(res => {
			...
			addStatisticsData() {
				...
				let sql =
					`'${obg.exhId}','${obg.exhName}','${obg.doorId}','${obg.loginName}','${obg.loginPhone}','${obg.dayTime}','${obg.count}','${obg.createTime}'`;
