<template>
	<view>
			...
			<block v-if="options.type==2">
				...
				<view  class="roww rowsb border_bottom"
				 ...
				 v-for="(item,index) in list">
					...
					<view class="allline w-100 your-class" style="text-align: center;"
					>{{item.unionid}}</view>
		...
		<!--  @longtap="toStartUpload" -->
		<view class="leftbo yincang1"
		>
			离线数据上传
		</view>
	...
	export default {
		...
		methods: {
			...
			getZong(tableName){
				...
				DB.selectgetTotalCont1(tableName).then(res=>{
					console.log("总个数",res)
			...
			getYeji(tableName){
				...
				DB.selectTableData1_1(tableName,100,this.list.length).then(res=>{
			...
			updateBendiDB(info,index,isTrue){
				...
				DB.updateTableData("scan_error_info",where,'id',info.id).then(res=>{
...
<style>
	/* CSS code to make text wrap */
	.your-class {
	  overflow-wrap: break-word; /* Allows breaking long words onto a new line */
	  word-break: break-all;     /* Breaks words when necessary to avoid overflow */
	  white-space: normal;       /* Default spacing behavior, allowing text wrapping */
	}
